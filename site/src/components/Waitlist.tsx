...
export function Waitlist() {
  ...
  return (
    <section id="waitlist" className="py-16 sm:py-20 px-6 relative">
      {/* Background glow */}
      <div
      ...
      <div ref={ref} className="reveal relative z-10 max-w-lg mx-auto text-center">
        ...
        {status === 'success' ? (
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg px-6 py-5">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 bg-[#1A1A1A] border border-neutral-600/60 hover:border-neutral-500 focus:border-gold-500/50 rounded-lg px-5 py-3.5 text-white font-body text-sm placeholder-neutral-500 outline-none transition-colors"
            />
            <button
              type="submit"
              disabled={status === 'loading'}
              className="glow-cta px-8 py-3.5 bg-gold-500 hover:bg-gold-400 disabled:opacity-60 text-[#0A0A0A] font-body font-semibold text-sm tracking-wide rounded-lg transition-colors flex-shrink-0"
            >
        ...
        <p className="mt-5 text-xs text-neutral-600 font-body">
